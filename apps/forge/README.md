...
- `src/fixes.ts`: inline fixes, written with `api.asUser()` so Jira permissions and history stay with the person clicking. Set estimate overwrites every story points field that already holds a value (sites often carry two), because the scan reads the first one with a number. `src/permissions.ts`: Jira admin / project admin checks that gate the Settings tabs.
